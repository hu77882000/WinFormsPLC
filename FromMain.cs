...
namespace WinFormsPLC
{
    public partial class FrmMain : Form
    {
        ...
        {
            OperateResult<int> read = await readWriteNet.ReadInt32Async(txtRegisterAddress3.Text);
            ...
            if (date == txtRegisterAddress3.Text)
            {
                ...
                if (Convert.ToInt32(HelpToolClass.Plc_Address_Binary_Index) == 1)
                {
                    ...
                    OperateResult write = await readWriteNet.WriteAsync(txtRegisterAddress2.Text, HelpToolClass.Plc_Address_Return); //修改到工作中的状态
        ...
        private async void DataComparisonAsync(string msg)
        {
            #region
            /*            //编码处理转换
            ...
            if (isbool)
            {
                ...
                OperateResult write = await readWriteNet.WriteAsync(txtRegisterAddress4.Text, content);
            }
            else
            {
