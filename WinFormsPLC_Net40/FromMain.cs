...
namespace WinFormsPLC_Net40
{
    public partial class FrmMain : Form
    {
        ...
        private void FrmMain_Load(object sender, EventArgs e)
        {
            ...
            {
               DialogResult dr = MessageBox.Show("初始化配置信息加载失败，TCP/串口加载失败，是否重新加载","错误",MessageBoxButtons.RetryCancel,MessageBoxIcon.Error);
        ...
        private void SP_ReadData_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            Invoke(new Action(() =>
            {
                ...
                string ProductName = "商品001";
                string str1 = "select * from PT_BarCode where ItemCode ='" + receiveStr.Trim() + "';";
                if (SqliteHelper.sqlQuery(str1) == 0)
                {
                    string str = string.Format("insert into PT_BarCode values('{0}','{1}','{2}','{3}','{4}','{5}')", Guid.NewGuid().ToString(), receiveStr.Trim(), ProductName, "李四", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff"), "0");
                    if (SqliteHelper.OperationDB(str))
                    {
                        LogHelper.Info(receiveStr.Trim() + "数据库写入成功，执行sql：" + str);
                        Console.WriteLine(receiveStr.Trim() + "数据库写入成功，执行sql：" + str);
                    }
                    else
                    {
                        LogHelper.Info(receiveStr.Trim() + "数据库录入失败");
                        Console.WriteLine(receiveStr.Trim() + "数据库录入失败");
                    }
                }
                else
                {
                    LogHelper.Info(receiveStr.Trim() + "数据库录入失败，数据重复扫描");
                    Console.WriteLine(receiveStr.Trim() + "数据库录入失败，数据重复扫描");
                }
                // Write(receiveStr.Trim());   ///数据写入
        ...
        private void SP_ReadData_DataReceived1(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            Invoke(new Action(() =>
            {
                ...
                string str = "select * from  PT_BarCode  where ItemCode = " + receiveStr.Trim();
                if (SqliteHelper.sqlQuery(str) > 0)
                {
                    LogHelper.Info(str + "数据库比对成功");
                    Console.WriteLine(str + "数据库比对成功");
                ...
                {
                    LogHelper.Info(str + "数据库数据不存在，比对失败");
                    Console.WriteLine(str + "数据库数据不存在，比对失败");
                }
        ...
        private void btnSend_ClickAsync(object sender, EventArgs e)
        {
            ...
            {
                Console.WriteLine ("16进制数的格式或长度不正确，请检查格式后重新发送。");
        ...
        private void SettingControls(int canUse)
        {
            if (canUse == 0)
            {
                ...
                uiCheckBoxGroup6.Enabled = false;
                /*txtSend.Enabled = true;
                btnCycleSend.Enabled = true;
                btnCycleReceive.Enabled = true;
                txtReceive.Enabled = true;
